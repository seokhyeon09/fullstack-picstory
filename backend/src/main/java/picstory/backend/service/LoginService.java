...
public class LoginService {
    ...
    private static final String LOGIN_MEMVER_ID = "LOGIN_MEMBER_ID";
    ...
    public MemberResponse login(LoginRequest request, HttpSession session){
        ...
        if(!passwordEncoder.matches(request.password(), member.getPasswordHash())){
            throw new IllegalArgumentException("이메일 또는 비밀번호가 올바르지않습니다.");
        }

        session.setAttribute(LOGIN_MEMVER_ID, member.getId());

        return MemberResponse.from(member);
    }

    public MemberResponse me(HttpSession session){
        Long memberId = (Long) session.getAttribute(LOGIN_MEMVER_ID);
        if(memberId==null){
